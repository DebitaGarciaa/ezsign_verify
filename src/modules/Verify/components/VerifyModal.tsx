                    ...
                    return hasIssue ? (
                     ...
                     <div className="h-full flex-1 flex flex-col items-center justify-center p-6 animate-in fade-in zoom-in duration-300">
                      <div className="bg-[#F25F5C] text-white p-10 rounded-xl flex flex-col items-center text-center shadow-lg w-full max-w-[720px]">
                        ...
                        {/* TARUH KODE "SOLUSI 2" DI SINI (Ganti mb-8 yang lama) */}
                            <div className="mb-8">
          ...
          <div className="bg-[#F25F5C] rounded-full p-1.5 flex items-center justify-center">
            <X size={28} className="text-white" strokeWidth={3} />
                      ...
                      <div className="flex flex-col gap-6 w-full animate-in slide-in-from-right-2 duration-300">
                        ...
                        {[
                          { 
                            text: 'Dokumen dimodifikasi dan isi dokumen tidak dijamin integritasnya', 
                            icon: <FileText size={18} />, 
                            // Gabungkan semua kemungkinan status sukses dari log API kamu
                            valid: activeSig?.["File hash Validation"]?.toLowerCase().includes("valid") || 
                                  activeSig?.["Signature"]?.toLowerCase().includes("verified") ||
                                  activeSig?.["code"] == 200
                          },
                          { 
                            text: 'Dokumen dengan sertifikat digital terpercaya', 
                            icon: <ShieldCheck size={18} />, 
                            // Pastikan issuer tidak mengandung sensor 'untrusted'
                            valid: activeSig?.["Certificate Status"]?.toLowerCase().includes("not expire") && 
                        ...
                        <div className="flex flex-col gap-6 mt-6">
                          <div className="border border-gray-200 rounded-lg p-6 bg-white shadow-sm relative">
                            ...
                            <div className="grid grid-cols-[200px_1fr] gap-y-3 text-[12px] text-gray-600 pt-2">
                              ...
                              <div>Alasan</div>
                              <div>: {activeSig["Reason"] || activeSig["Alasan"] || "-"}</div>
                              ...
                              <div className="text-gray-800">
                                : {(() => {
                                  ...
                                  // 2. Jika kosong, cari field 'timestamp' atau 'stempel_waktu' yang mungkin ada di level atas
                                  const backupTime = getField(["timestamp", "stempel_waktu", "Signing Time", "timestamp signature"]);
                                  
                                  if (backupTime !== "-" && backupTime !== "verified") {
                                      return formatDate(backupTime);
                                  }
